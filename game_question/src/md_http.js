...
md_http.use("/question", function(req, res, next){
	...
	// if(selfSign != sign){
	// 	return res.josn({ret_code : -4, ret_msg :"sign is error"});
	// }
	next();	
});

md_http.use("/question", require("./router/question"))
...
md_http.use(function(err, req, res, next) {
	g_App.log_http_error.error(err.message)
  	res.send("run health!!")
});
